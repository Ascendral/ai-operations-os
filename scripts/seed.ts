...
function seedTasks(stores: ReturnType<typeof createStores>): Task[] {
  const taskDefs: Array<Parameters<typeof createTask>[0]> = [
    ...
    {
      source: "email",
      title: "Re: Q1 Revenue Report needs review",
      body: "Hi team, the Q1 revenue report is attached. Please review the figures in section 3 before our board meeting on Friday.",
      intent: "reply",
      priority: "high",
      status: "pending",
    },
    {
      source: "email",
      title: "Customer complaint: order #8842 damaged",
      body: "My order arrived with a cracked screen. I need a replacement or refund ASAP. Order number is 8842.",
      intent: "escalate",
      priority: "urgent",
      status: "running",
    },
    {
      ...
      body: "We are interested in exploring a strategic partnership with your company. Could we schedule a call next week?",
      intent: "reply",
      priority: "normal",
      status: "pending",
    },
    {
      source: "email",
      title: "Weekly newsletter draft for approval",
      body: "Attached is the draft for this week's newsletter. Please approve or suggest edits by end of day.",
      intent: "reply",
      priority: "low",
    ...
    // Calendar tasks (4)
    {
      ...
      body: "Block 2 hours next Thursday for the quarterly product roadmap review with engineering leads.",
      intent: "schedule",
      priority: "high",
      status: "planned",
    },
    {
      source: "calendar",
      title: "Reschedule investor call to Friday",
      body: "The investor call originally set for Wednesday needs to move to Friday 2pm EST.",
      intent: "schedule",
      priority: "urgent",
      status: "pending",
    },
    {
      source: "calendar",
      title: "Team standup recurring meeting setup",
      body: "Create a recurring daily standup at 9:15am for the platform engineering team.",
      intent: "schedule",
      priority: "normal",
      status: "completed",
    },
    {
      source: "calendar",
      title: "Cancel deprecated sprint planning",
      body: "The old sprint planning meeting on Mondays is no longer needed. Please cancel all future occurrences.",
      intent: "schedule",
      priority: "low",
      status: "completed",
    },
    ...
    {
      source: "social",
      title: "Respond to viral product mention on X",
      body: "A tech influencer with 500k followers just posted about our product. Draft a thank-you reply and repost.",
      intent: "post",
      priority: "urgent",
      status: "running",
    },
    {
      source: "social",
      title: "Schedule launch announcement post",
      body: "Prepare and schedule the v2.0 launch announcement across X, LinkedIn, and the company blog.",
      intent: "post",
      priority: "high",
      status: "awaiting_approval",
    },
    {
      source: "social",
      title: "Monitor brand mentions this week",
      body: "Track and summarize all brand mentions across social platforms for the weekly report.",
      intent: "ignore",
      priority: "normal",
      status: "planned",
    },
    {
      source: "social",
      title: "Reply to customer support question on X",
      body: "User @janedoe asked about our refund policy. Draft a helpful public reply.",
      intent: "reply",
      priority: "normal",
      status: "pending",
    },
    ...
    {
      source: "store",
      title: "Process refund for order #12045",
      body: "Customer requested a full refund for order #12045. Item was returned in original packaging within 30 days.",
      intent: "refund",
      priority: "high",
      status: "awaiting_approval",
    },
    {
      source: "store",
      title: "Fulfill bulk order for Acme Corp",
      body: "150 units of SKU-7721 need to be shipped to Acme Corp warehouse by end of week. Priority shipping required.",
      intent: "fulfill",
      priority: "urgent",
      status: "running",
    },
    {
      source: "store",
      title: "Update inventory for restocked items",
      body: "Warehouse confirmed receipt of 500 units each for SKU-3301, SKU-3302, and SKU-3303.",
      intent: "fulfill",
      priority: "normal",
      status: "completed",
    },
    {
      source: "store",
      title: "Investigate payment failure on order #12100",
      body: "Payment gateway returned error code E-4422 for order #12100. Customer card may have been declined.",
      intent: "escalate",
      priority: "high",
      status: "failed",
    },
    ...
    {
      source: "manual",
      title: "Review CORD safety policy thresholds",
      body: "The current CORD thresholds may be too conservative. Review the allow/challenge/block boundaries and propose adjustments.",
      intent: "unknown",
      priority: "normal",
      status: "pending",
    },
    {
      source: "manual",
      title: "Audit last month receipt chain integrity",
      body: "Run a full verification of the receipt chain for all actions executed in the previous calendar month.",
      intent: "unknown",
      priority: "low",
      status: "planned",
    },
    {
      source: "manual",
      title: "Onboard new connector: Slack integration",
      body: "Implement and test the Slack connector module. Should support read, post, and thread-reply operations.",
      intent: "unknown",
      priority: "high",
      status: "running",
    },
    {
      source: "manual",
      title: "Generate compliance report for Q1",
      body: "Compile all approval decisions, CORD scores, and receipt chain summaries into the quarterly compliance PDF.",
      intent: "unknown",
      priority: "normal",
      status: "pending",
    },
...
function seedWorkflows(
  ...
  const wf3 = createWorkflowRun(tasks[12].id, "store-refund", [
    ...
    createStep("codebot", "evaluate_refund", {
      reason: "returned in packaging",
    }),
  ...
  const wf5 = createWorkflowRun(tasks[4].id, "calendar-schedule", [
    createStep("google-calendar", "find_availability", {
      date: "2026-03-12",
      duration: 120,
    }),
    createStep("codebot", "suggest_time", { preference: "afternoon" }),
    createStep("google-calendar", "create_event", {
      title: "Product Roadmap Review",
    }),
    createStep("gmail", "send_invite", {
      attendees: ["eng-leads@company.com"],
    }),
  ]);
  wf5.state = "queued";
...
function seedReceipts(
  ...
  const receiptInputs: Array<
    Omit<ActionReceipt, "hash" | "signature" | "prevHash">
  > = [
    {
  ...
  for (const scenario of scenarios) {
    ...
    for (let i = 0; i < scenario.episodes.length; i++) {
      ...
      const episode: LearningEpisode = {
        ...
        createdAt: new Date(
          baseTime.getTime() + episodeCount * 3600000,
        ).toISOString(),
      };
...
export async function runSeed(reset: boolean = false): Promise<{
  ...
  if (reset && fs.existsSync(DB_PATH)) {
    ...
    console.log(
      `${c.yellow}Reset:${c.reset} Deleted existing database at ${DB_PATH}`,
    );
  }
...
if (require.main === module || process.argv[1]?.endsWith("seed.ts")) {
  runSeed(isReset)
    .then((counts) => {
      ...
      console.log(
        `  ${c.cyan}SPARK:${c.reset}     ${counts.sparkEpisodes} episodes, ${counts.sparkInsights} insights`,
      );
      console.log();
