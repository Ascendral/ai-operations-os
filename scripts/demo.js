...
function step(icon, label, detail) {
  console.log(
    `  ${icon}  ${c.bold}${label}${c.reset}  ${c.dim}${detail || ""}${c.reset}`,
  );
}
  ...
  switch (type) {
    case "task_created":
      step(
        "📥",
        "Task Created",
        `id=${task?.id?.slice(0, 8)}...  "${task?.title}"`,
      );
      break;
    ...
    case "workflow_started":
      step(
        "⚡",
        "Workflow Started",
        `type=${c.blue}${run?.workflowType}${c.reset}  steps=${run?.steps?.length}`,
      );
      break;
    case "step_evaluating":
      step(
        "🔍",
        "Evaluating",
        `${c.cyan}${s?.connector}.${s?.operation}${c.reset}`,
      );
      break;
    case "step_allowed": {
      ...
      step(
        "📋",
        "Policy Gate",
        `${c.green}autonomous${c.reset}  — read-only, no approval needed`,
      );
      break;
    }
    case "step_approval_needed":
      step(
        "🛡️",
        "CORD Safety",
        `${c.green}ALLOW${c.reset}  score=${s?.cordScore ?? 0}/99`,
      );
      step(
        "📋",
        "Policy Gate",
        `${c.yellow}approval required${c.reset}  — write operation`,
      );
      step(
        "🔔",
        "Approval Request",
        `${c.yellow}risk=${approval?.risk}${c.reset}  "${approval?.reason}"`,
      );
      break;
    case "step_approved":
      step(
        "✅",
        "Approved",
        `${c.green}user approved${c.reset}  ${c.dim}(auto in demo)${c.reset}`,
      );
      break;
    case "step_blocked":
    ...
    case "step_completed":
      step(
        "✔️",
        "Done",
        `${c.green}${s?.connector}.${s?.operation}${c.reset}  ${s?.durationMs ?? 0}ms`,
      );
      break;
    case "workflow_completed":
      step(
        "🏁",
        "Workflow Complete",
        `${c.green}${c.bold}SUCCESS${c.reset}  ${run?.steps?.length} steps executed`,
      );
      break;
...
async function runScenario(title, payload) {
  ...
  if (payload.event?.subject)
    step("   ", "", `subject: "${payload.event.subject}"`);
...
async function runSimulation(title, payload) {
  ...
  step(
    "🔮",
    "Dry-Run Simulation",
    `source=${c.cyan}${payload.source}${c.reset}  "${payload.title}"`,
  );
  console.log();
  ...
  for (const s of data.steps) {
    ...
    const policy = auto
      ? `${c.green}autonomous${c.reset}`
      : `${c.yellow}needs approval${c.reset}`;
    const safety =
      s.safetyDecision === "ALLOW"
        ? `${c.green}ALLOW${c.reset}`
        : `${c.red}${s.safetyDecision}${c.reset}`;
  ...
  console.log(
    `  ${c.bold}Summary:${c.reset} ${summary.totalSteps} steps | ${c.green}${summary.autoSteps} auto${c.reset} | ${c.yellow}${summary.approvalSteps} approval${c.reset} | ${c.red}${summary.blockedSteps} blocked${c.reset}`,
  );
  console.log();
}
...
async function main() {
  ...
  console.log(
    `${c.bold}${c.cyan}  │       AI OPERATIONS OS — LIVE DEMO          │${c.reset}`,
  );
  console.log(
    `${c.bold}${c.cyan}  │  Policy Gates · CORD Safety · Audit Trail   │${c.reset}`,
  );
  console.log(
    `${c.bold}${c.cyan}  └─────────────────────────────────────────────┘${c.reset}`,
  );
  ...
  try {
    ...
    step(
      "💚",
      "Server Online",
      `v${health.version}  uptime=${Math.floor(health.uptime)}s`,
    );
  } catch {
    console.error(
      `${c.red}  ✗ Server not running. Start with: npm run dev --workspace=apps/ops-api${c.reset}`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `    ${c.green}•${c.reset} Policy gates: read ops run autonomously, writes need approval`,
  );
  ...
  console.log(
    `    ${c.green}•${c.reset} Cryptographically signed execution receipts`,
  );
  ...
  step(
    "📦",
    "npm",
    "npmjs.com/package/codebot-ai  ·  npmjs.com/package/cord-engine",
  );
  console.log();
}
