...
import { GmailConnector } from "@ai-operations/ops-connectors";
...
async function listInbox(ctx: any): Promise<void> {
  ...
  if (!gmail) {
    sendError(
      res,
      401,
      "Gmail not connected. Authorize at GET /api/oauth/google/url",
    );
    return;
  }
...
async function processMessage(ctx: any): Promise<void> {
  ...
  const sparkPrediction = sparkPredict(
    messageId,
    "gmail",
  ...
  const policyResult = ruleEngine.evaluate("gmail", operation, {
    source: "email",
  });
  ...
  const sparkResult = sparkLearn({
    ...
    wasApproved:
      !!approvalResult?.decision && approvalResult.decision === "approved",
    durationMs: Date.now() - execStart,
