...
describe("requiresElevatedRole", () => {
  test("DELETE always requires elevated role", () => {
    ...
    expect(requiresElevatedRole("DELETE", "/api/auth/credentials/456")).toBe(
      true,
    );
  ...
  test("POST to approval decide requires elevated role", () => {
    expect(requiresElevatedRole("POST", "/api/approvals/abc-123/decide")).toBe(
      true,
    );
    expect(requiresElevatedRole("POST", "/api/approvals/some-id/decide")).toBe(
      true,
    );
  });
