...
export interface PipelineConfig {
  ...
  requestApproval: (
    approval: Approval,
  ) => Promise<"approved" | "denied" | "modified">;

  /** Build workflow steps for a given task */
  buildWorkflow: (task: Task) => {
    workflowType: string;
    steps: Array<{
      connector: string;
      operation: string;
...
export async function* runPipeline(
  ...
  const task = createTask({
    ...
    title:
      ...
      `${source} event`,
    body:
      (eventData.body as string) || (eventData.snippet as string) || undefined,
    sourceId:
      (eventData.messageId as string) ||
      (eventData.id as string)?.toString() ||
      undefined,
    metadata: eventData,
  });
  ...
  for (const step of run.steps) {
    ...
    if (config.spark?.beforeStep) {
      sparkPrediction = config.spark.beforeStep(
        step.id,
        run.id,
        step.connector,
        step.operation,
    ...
    // ── 4b. CORD safety gate ─────────────────────────────────────
    const safety = config.evaluateSafety(
      step.connector,
      step.operation,
      step.input,
    );
    ...
    if (needsApproval) {
      ...
      if (decision === "denied") {
        ...
        const sparkEpisodeDenied = config.spark?.afterStep?.(
          step,
          run.id,
          wasApproved,
        );
    ...
    const result = await config.executeConnector(
      step.connector,
      step.operation,
      step.input,
    );
    ...
    if (!result.success) {
      ...
      const sparkEpisodeFailed = config.spark?.afterStep?.(
        step,
        run.id,
        wasApproved,
      );
