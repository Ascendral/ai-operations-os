...
export async function handleEmailTriage(
  job: QueueJob<EmailTriageData>,
): Promise<unknown> {
  const { taskId, from, subject } = job.data;
  console.log(
    `[email.triage] Processing: "${subject}" from ${from} (task: ${taskId})`,
  );
...
export async function handleEmailReply(
  job: QueueJob<EmailReplyData>,
): Promise<unknown> {
  const { taskId, to, subject } = job.data;
  console.log(
    `[email.reply] Drafting reply to ${to}: "${subject}" (task: ${taskId})`,
  );
...
function classifyEmailPriority(subject: string, from: string): string {
  ...
  if (
    ...
    return "urgent";
  if (text.includes("important") || text.includes("action required"))
    return "high";
