 ...
 */
export async function handleCalendarCheck(
  job: QueueJob<CalendarCheckData>,
): Promise<unknown> {
  const { taskId, summary, startTime, endTime, organizer } = job.data;
  console.log(
    `[calendar.check] Checking: "${summary}" from ${organizer} (task: ${taskId})`,
  );
 ...
 */
export async function handleCalendarRespond(
  job: QueueJob<CalendarRespondData>,
): Promise<unknown> {
  const { taskId, eventId, response } = job.data;
  console.log(
    `[calendar.respond] Responding ${response} to event ${eventId} (task: ${taskId})`,
  );
