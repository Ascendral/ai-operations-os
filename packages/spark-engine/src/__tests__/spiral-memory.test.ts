...
describe("EssenceExtractor", () => {
  test("extracts topics from text", () => {
    ...
    expect(
      topics.some(
        (t) =>
          t.includes("financ") || t.includes("payment") || t.includes("refund"),
      ),
    ).toBe(true);
  });
  ...
  test("analyzes positive sentiment", () => {
    const { extractor } = createAll();
    const result = extractor.analyzeSentiment(
      "The system is reliable and accurate with excellent results.",
    );
  ...
  test("analyzes negative sentiment", () => {
    const { extractor } = createAll();
    const result = extractor.analyzeSentiment(
      "The system failed with errors and dangerous unstable behavior.",
    );
  ...
  test("handles negation", () => {
    const { extractor } = createAll();
    const result = extractor.analyzeSentiment(
      "The system is not good and not reliable.",
    );
    expect(result.valence).toBe("negative");
  });
  ...
  test("returns neutral for non-sentiment text", () => {
    const { extractor } = createAll();
    const result = extractor.analyzeSentiment(
      "The function returns a number between zero and ten.",
    );
  ...
  test("extracts relationships", () => {
    ...
    const rels = extractor.extractRelationships(
      "volatility causes instability and risk leads to failure",
    );
  ...
  test("generates gist from long text", () => {
    const { extractor } = createAll();
    const longText =
      "The financial operations have been volatile recently. " +
  ...
  test("importance scoring weights SENTINEL categories higher", () => {
    const { extractor } = createAll();
    const financialEssence = extractor.extract(
      "Payment processing status update",
      { categories: ["financial"] },
    );
    const readonlyEssence = extractor.extract("Search query status update", {
      categories: ["readonly"],
    });
    expect(financialEssence.importance).toBeGreaterThan(
      readonlyEssence.importance,
    );
  });
});
...
describe("MemoryTokenManager", () => {
  ...
  test("merges tokens into composite", () => {
    const { tokenManager, store } = createAll();
    const token1 = tokenManager.createFromTurn(
      makeTurn({ content: "Email operations are reliable and stable." }),
    );
    const token2 = tokenManager.createFromTurn(
      makeTurn({ content: "Email communication performing well today." }),
    );
  ...
  test("archives weak tokens", () => {
    ...
    store.updateMemoryTokenStrength(
      token.id,
      0.01,
      0,
...
describe("SpiralLoop", () => {
  test("spiral pass reinforces related tokens", () => {
    ...
    expect(
      result.tokensReinforced + result.tokensWeakened,
  ...
  test("spiral pass creates edges between related tokens", () => {
    ...
    expect(result.edgesCreated + result.edgesReinforced).toBeGreaterThanOrEqual(
      0,
    );
  });
  ...
  test("topic similarity: identical topics return high score", () => {
    ...
    const a = {
      topics: ["email", "communication", "reliable"],
  ...
  test("empty memory returns empty spiral pass result", () => {
    ...
    const token = tokenManager.createFromTurn(
      makeTurn({ content: "Hello world" }),
    );
...
describe("FeedbackIntegrator", () => {
  test("onConversationTurn creates token and runs spiral", () => {
    ...
    const tokens = store.listMemoryTokens({
      type: "conversation",
...
describe("Spiral Memory Integration", () => {
  test("full flow: turns → tokens → spiral → reconstruct", () => {
    ...
    integrator.onConversationTurn(
      makeTurn({
        content:
          "The email communication system processed 50 messages today reliably.",
      }),
    );
    integrator.onConversationTurn(
      makeTurn({
        content:
          "Financial payment operations had an error during refund processing.",
      }),
    );
    integrator.onConversationTurn(
      makeTurn({
        content:
          "Email communication continues to perform with excellent accuracy.",
      }),
    );
  ...
  test("spiral reinforcement increases token strength over time", () => {
    ...
    const content =
      "The email communication system is performing reliably and accurately.";
    integrator.onConversationTurn(makeTurn({ content }));
    integrator.onConversationTurn(
      makeTurn({ content: "Email communication accuracy remains excellent." }),
    );
    integrator.onConversationTurn(
      makeTurn({
        content: "Communication via email continues to be reliable.",
      }),
    );
...
describe("SpiralLoop — edge decay", () => {
  it("exponentially decays edges during maintenance pass", () => {
    ...
    const token1 = tokenManager.createFromTurn(
      makeTurn({ content: "Email communication system reliable performance" }),
    );
    const token2 = tokenManager.createFromTurn(
      makeTurn({ content: "Email communication performance improvements" }),
    );
    ...
    expect(
      maintenanceResult.edgesDecayed + maintenanceResult.edgesPruned,
    ).toBeGreaterThan(0);
  });
  ...
  it("prunes edges below threshold", () => {
    ...
    const token1 = tokenManager.createFromTurn(
      makeTurn({ content: "Email communication reliable system" }),
    );
    const token2 = tokenManager.createFromTurn(
      makeTurn({ content: "Email communication system updates" }),
    );
...
describe("SpiralLoop — exponential token decay", () => {
  it("uses exponential decay for token strength", () => {
    ...
    const token = tokenManager.createFromTurn(
      makeTurn({ content: "Financial transaction processing" }),
    );
...
describe("MemoryTokenManager — autoMerge", () => {
  it("merges same-type tokens with >80% topic overlap", () => {
    ...
    const token1 = tokenManager.createFromTurn(
      makeTurn({
        content:
          "Email communication system performance is reliable and stable",
      }),
    );
    const token2 = tokenManager.createFromTurn(
      makeTurn({
        content:
          "Email communication system performance is reliable and consistent",
      }),
    );
...
describe("SpiralLoop — weighted cosine similarity", () => {
  ...
  it("rare shared topics score higher than common ones", () => {
    ...
    const commonSim = spiral.computeTopicSimilarity(
      essenceCommon,
      essenceCommon2,
    );
...
describe("ContextReconstructor — adaptive depth", () => {
  it("expands depth when too few tokens at initial depth", () => {
    ...
    const token3 = tokenManager.createFromTurn(
      makeTurn({
