...
describe("PersonalityEngine", () => {
  ...
  describe("response modulation", () => {
    ...
    it("removes hedging words when directness > 0.65", () => {
      ...
      const result = engine.modulateResponse(
        "I think perhaps the system maybe needs attention.",
      );
  ...
  describe("consistency scoring", () => {
    ...
    it("is high with consistent inputs", () => {
      for (let i = 0; i < 20; i++) {
        engine.evolve(
          defaultContext({ topicDiversity: 5, emotionalValence: 0.3 }),
        );
      }
        ...
        const erratic =
          i % 2 === 0
            ? defaultContext({
                ...
                emotionalMomentum: "improving",
              })
            : defaultContext({
                topicDiversity: 0,
                emotionalValence: -0.9,
                emotionalMomentum: "declining",
              });
  ...
  describe("persistence", () => {
    it("saves and restores personality profile", () => {
      ...
      for (let i = 0; i < 10; i++) {
        engine.evolve(
          defaultContext({ topicDiversity: 8, emotionalValence: 0.5 }),
        );
      }
      ...
      expect(profileAfter.playfulness).toBeCloseTo(
        profileBefore.playfulness,
        4,
      );
    });
  });
