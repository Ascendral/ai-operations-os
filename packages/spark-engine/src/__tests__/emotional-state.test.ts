...
describe("EmotionalStateEngine", () => {
  ...
  describe("EMA valence tracking", () => {
    ...
    it("converges toward negative with repeated negative input", () => {
      for (let i = 0; i < 10; i++) {
        engine.updateFromEssence(
          makeEssence({ sentiment: "negative", sentimentIntensity: 0.8 }),
        );
      }
      const state = engine.getState();
    ...
    it("stays near zero with neutral input", () => {
      for (let i = 0; i < 10; i++) {
        engine.updateFromEssence(
          makeEssence({ sentiment: "neutral", sentimentIntensity: 0.5 }),
        );
      }
      const state = engine.getState();
    ...
    it("blends mixed signals slightly positive", () => {
      engine.updateFromEssence(
        makeEssence({ sentiment: "mixed", sentimentIntensity: 0.6 }),
      );
      const state = engine.getState();
  ...
  describe("momentum detection", () => {
    it("detects improving momentum", () => {
      ...
      for (let i = 0; i < 5; i++) {
        engine.updateFromEssence(
          makeEssence({ sentiment: "negative", sentimentIntensity: 0.3 }),
        );
      }
      for (let i = 0; i < 5; i++) {
        engine.updateFromEssence(
    ...
    it("detects declining momentum", () => {
      ...
      for (let i = 0; i < 5; i++) {
        engine.updateFromEssence(
          makeEssence({ sentiment: "negative", sentimentIntensity: 0.9 }),
        );
      }
      const state = engine.getState();
    ...
    it("detects stable momentum with consistent input", () => {
      ...
      for (let i = 0; i < 20; i++) {
        engine.updateFromEssence(
          makeEssence({ sentiment: "positive", sentimentIntensity: 0.5 }),
        );
      }
      const state = engine.getState();
      expect(state.momentum).toBe("stable");
  ...
  describe("persistence", () => {
    it("saves and restores emotional state", () => {
      ...
      expect(stateAfterRestore.valence).toBeCloseTo(
        stateBeforeRestore.valence,
        4,
      );
      expect(stateAfterRestore.highEmotionCount).toBe(
        stateBeforeRestore.highEmotionCount,
      );
    });
  });
