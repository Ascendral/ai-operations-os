...
function printHelp(): void {
  ...
  console.log(
    `  ${c.cyan}health${c.reset} [--url URL]     Check API server health status`,
  );
  console.log(`                          Default URL: http://localhost:3100`);
  console.log(
    `  ${c.cyan}demo${c.reset}                   Load demo seed data into the database`,
  );
  console.log(
    `  ${c.cyan}receipts verify${c.reset}        Verify receipt chain integrity`,
  );
  console.log(`  ${c.cyan}version${c.reset}                Print CLI version`);
  console.log(
    `  ${c.cyan}help${c.reset}                   Print this usage information`,
  );
  console.log();
  console.log(`${c.bold}EXAMPLES${c.reset}`);
  console.log(`  ${c.dim}$ ai-ops health${c.reset}`);
  console.log(
    `  ${c.dim}$ ai-ops health --url http://production.example.com:3100${c.reset}`,
  );
