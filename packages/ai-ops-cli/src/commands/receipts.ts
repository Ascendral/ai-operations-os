...
export async function receiptsVerify(): Promise<void> {
  ...
  try {
    ...
    if (rows.length === 0) {
      ...
      console.log(
        `${c.dim}Run "ai-ops demo" first to load seed data.${c.reset}`,
      );
      return;
    }
    ...
    if (result.valid) {
      console.log(
        `  ${c.bold}Chain Status:${c.reset}    ${c.green}${c.bold}VALID${c.reset}`,
      );
      console.log();
      console.log(
        `  ${c.green}All ${receipts.length} receipts verified -- hashes linked, signatures match.${c.reset}`,
      );
    } else {
      console.log(
        `  ${c.bold}Chain Status:${c.reset}    ${c.red}${c.bold}INVALID${c.reset}`,
      );
      if (result.brokenAt !== undefined) {
        console.log(
          `  ${c.bold}Broken At:${c.reset}       Receipt #${result.brokenAt + 1}`,
        );
      }
      if (result.reason) {
      ...
      console.log();
      console.log(
        `  ${c.red}Chain integrity compromised. Investigate the broken link.${c.reset}`,
      );
      process.exitCode = 1;
    }
