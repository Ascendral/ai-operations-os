...
export class GmailConnector extends BaseConnector {
  ...
  async execute(
    ...
    if (!this.accessToken) {
      return {
        success: false,
        error: "No Gmail access token configured. Run setup first.",
      };
    }
    ...
    try {
      switch (operation) {
        ...
        case "read":
          return await this.readMessage(input);
        case "send":
          return await this.sendMessage(input);
        case "reply":
          return await this.replyToThread(input);
        case "label":
          return await this.manageLabels(input);
        case "search":
  ...
  private async replyToThread(
    ...
    const threadRes = await this.gmailFetch(
      `/threads/${threadId}?format=metadata&metadataHeaders=From&metadataHeaders=Subject&metadataHeaders=Message-ID`,
    );
