...
function makeReceiptData(
  overrides: Partial<Omit<ActionReceipt, "hash" | "signature">> = {},
) {
  return {
    id: "receipt-1",
    actionId: "action-1",
...
describe("computeReceiptHash", () => {
  ...
  it("changes hash when cordReasons differ", () => {
    const h1 = computeReceiptHash(
      makeReceiptData({ cordReasons: ["reason A"] }),
    );
    const h2 = computeReceiptHash(
      makeReceiptData({ cordReasons: ["reason B"] }),
    );
    expect(h1).not.toBe(h2);
  });
});
...
describe("verifyReceiptChain", () => {
  ...
  it("returns valid for a properly chained series of receipts", () => {
    ...
    const r2 = makeSignedReceipt({
      id: "r-2",
      actionId: "action-2",
      prevHash: r1.hash,
    });
    const r3 = makeSignedReceipt({
      id: "r-3",
      actionId: "action-3",
      prevHash: r2.hash,
    });
