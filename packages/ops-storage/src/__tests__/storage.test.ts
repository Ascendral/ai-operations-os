...
describe("TaskStore", () => {
  ...
  it("update() merges partial updates and sets updatedAt", () => {
    ...
    const updated = store.update("task-update", {
      status: "completed",
      priority: "high",
    });
...
describe("ApprovalStore", () => {
  ...
  it("listPending() returns only undecided approvals", () => {
    ...
    expect(pending).toHaveLength(2);
    expect(
      pending.every((a) => a.decision === undefined || a.decision === null),
    ).toBe(true);
  });
  ...
  it('decide() with "modified" stores modifications', () => {
    ...
    store.decide("apr-mod", "modified", "admin", {
...
describe("WorkflowStore", () => {
  ...
  it("listRuns() filters by state", () => {
    store.saveRun(
      makeWorkflowRun("parent-task", { id: "r1", state: "queued" }),
    );
    store.saveRun(
      makeWorkflowRun("parent-task", { id: "r2", state: "completed" }),
    );
    store.saveRun(
      makeWorkflowRun("parent-task", { id: "r3", state: "queued" }),
    );
  ...
  it("updateRun() updates run state", () => {
    store.saveRun(
      makeWorkflowRun("parent-task", { id: "run-upd", state: "queued" }),
    );
  ...
  it("saveStep() adds a new step to an existing run", () => {
    store.saveRun(
      makeWorkflowRun("parent-task", { id: "run-addstep", steps: [] }),
    );
  ...
  it("saveStep() updates an existing step in a run", () => {
    ...
    store.saveRun(
      makeWorkflowRun("parent-task", { id: "run-updstep", steps: [step] }),
    );

    store.saveStep("run-updstep", {
      ...step,
      status: "completed",
      durationMs: 100,
    });
