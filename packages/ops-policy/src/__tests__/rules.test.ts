  ...
  describe("basic rule matching", () => {
    ...
    it("matches using wildcard connector", () => {
      const config = makeConfig([
        makeRule({
          id: "all-reads",
          match: { connector: "*", operation: "read" },
          action: "auto",
        }),
      ]);
      const engine = new RuleEngine(config);
    ...
    it("matches using wildcard operation", () => {
      const config = makeConfig([
        makeRule({
          id: "deny-all-shopify",
          match: { connector: "shopify", operation: "*" },
          action: "deny",
        }),
      ]);
      const engine = new RuleEngine(config);
    ...
    it("matches by source context", () => {
      ...
      expect(
        engine.evaluate("gmail", "read", { source: "email" }).autonomy,
      ).toBe("auto");
      expect(
        engine.evaluate("gmail", "read", { source: "manual" }).autonomy,
      ).toBe("approve"); // default
    });
    ...
    it("matches by intent context", () => {
      ...
      expect(
        engine.evaluate("gmail", "send", { intent: "escalate" }).autonomy,
      ).toBe("deny");
      expect(
        engine.evaluate("gmail", "send", { intent: "reply" }).autonomy,
      ).toBe("approve");
    });
  });
